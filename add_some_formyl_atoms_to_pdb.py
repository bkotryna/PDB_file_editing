...
for one_chain in DNA_chains:
    ...
    # ie skip six c5 atoms in the beginning
    # (BTW looked up the number in the nucleosome info word.doc file)
    
    # check: how many C5 atoms there are (ie how many cytosines we have)
    print("For chain ", chain_name, ", there are ", c5_atoms.residue_name.count(), " DC residues.")
    print("For chain ", chain_name, ", there are ",
          atoms.residue_name.loc[(atoms['chain_id'] == chain_name) & (atoms['residue_name'] == 'DG') & (atoms['atom_name'] == 'C5')].count(),
          " DG residues.")
    
    # the narrowed down array of c5 atoms (lacks six atoms at the beginning)
    c5_atoms = c5_atoms.iloc[6:] 
...
print("After all manipulations, there are ", atoms.residue_name.loc[(atoms['residue_name'] == '5FC') & (atoms['atom_name'] == 'C5')].count(),
                                                       " C5 atoms in 5FC residues in the whole pdb.")
# ...and count number of C5 atoms in DC groups (aka number of DC groups) - just to compare & double check
print("After all manipulations, there are ", atoms.residue_name.loc[(atoms['residue_name'] == 'DC') & (atoms['atom_name'] == 'C5')].count(),
                                                       " C5 atoms in DC residues in the whole pdb.")
print("After all manipulations, there are ",
      atoms.residue_name.loc[(atoms['residue_name'] == 'DG') & (atoms['atom_name'] == 'C5')].count(),
                                                       " C5 atoms in DG residues in the whole pdb.")
